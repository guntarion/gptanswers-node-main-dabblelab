...
const documents = [
  "This app name is custom knoledge Wale.<|endoftext|>",
...
router.post("/", (req, res) => {
  ...
  const data = {
    max_tokens: 256,
    stop: ["\n", "\n\n"],
    temperature: 0.5, // Optional, adjust the temperature parameter according to your preference
    ...
    ], // Add the messages property
  };

  client.post(endpoint, data)
  .then((result) => {
    // Assuming result.data.choices[0].message.content contains the desired text
    const answerText = result.data.choices[0].message.content; // Correct way to access the content based on your log
  ...
  .catch((err) => {
    ...
    if (err.response) {
      ...
      res.send({
        answer: `Sorry, there was an API error. The error was '${err.response.data.message}'`, // Adjusted to log the error message
      });
    } else {
